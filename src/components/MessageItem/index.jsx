...
import { faPen, faPaperPlane, faTimes } from '@fortawesome/free-solid-svg-icons';
...
export class MessageItem extends React.Component {
  constructor(props) {
    ...
    this.state = {
      onEdit: false,
      message: this.props.cur.updatedAt ? this.props.cur.message : this.props.cur.customType,
    };
  }
  handleDeleteBtn = () => {
    ...
    return (
      <div className="message-item" >
        <div className="sender-img">
          <img src={cur.sender || cur.sender.profileUrl ? cur.sender.profileUrl : 'http://dxstmhyqfqr1o.cloudfront.net/images/icon-chat-04.png'} alt={cur.nickname} />
        </div>
        <div>
          <p className="sender-info">
            <span className="sender-nick">{cur.sender.nickname ? cur.sender.nickname : 'noname' }</span>
            { cur.updatedAt ?
              <span className="sending-date">Обновлено: {moment(cur.updatedAt).format('DD/MM/YY hh:mm a')}</span>
              :
          ...
          </p>
          {
            ...
            :
              <p className="message-text">{ cur.updatedAt ? cur.message : cur.customType }</p>
            }
        </div>
