...
describe('sendMessageSaga', () => {
  ...
  it('should send message', () => {
    ...
    expect(gen.next(sendRes).value).toEqual(
...
describe('sendFileMessageSaga', () => {
  ...
  const sendFileMessageSagaGroup = cloneableGenerator(sagas.sendFileMessageSaga)(actionGroup);

  it('should send file message group', () => {
    const gen = sendFileMessageSagaGroup.clone();
    gen.next();
    expect(gen.next().value).toEqual(call(sendFileMessage, ...actionGroup.fileMessageData));
    const sendRes = {
      channel: { channelType: 'group', url: 'https://url.com/' },
      message: { messageId: 1, message: 'text' }
    };
    gen.next(sendRes);
    expect(gen.next(sendRes.channel).value).toEqual(call(markAsReadSb, sendRes.channel));
